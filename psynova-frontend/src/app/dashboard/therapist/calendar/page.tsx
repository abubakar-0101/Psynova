...
const inputClass = [
  ...
  'focus:outline-none focus:ring-2 focus:ring-[#4A90D9]',
].join(' ');
    ...
    <DashboardShell navItems={therapistNav} title="Availability Calendar">
      <div className="max-w-2xl space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold" style={{ color: 'var(--dash-text)' }}>Weekly Availability</h2>
            <p className="text-sm" style={{ color: 'var(--dash-muted)' }}>Set your recurring weekly schedule</p>
          </div>
          ...
          <Card className="border-[#4A90D9]/30">
            <CardContent className="pt-5">
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div>
                  <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--dash-text)' }}>Day</label>
                  <select
                    value={formDay}
                ...
                <div>
                  <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--dash-text)' }}>Start</label>
                  <input
                    type="time"
                    value={formStart}
                ...
                <div>
                  <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--dash-text)' }}>End</label>
                  <input
                    type="time"
                    value={formEnd}
        ...
        <Card>
          <CardContent className="pt-5">
            {isLoading ? (
              ...
              <div className="text-center py-8" style={{ color: 'var(--dash-muted)' }}>
              ...
              <div className="space-y-4">
                {Object.entries(grouped).sort(([a], [b]) => DAYS.indexOf(a) - DAYS.indexOf(b)).map(([day, daySlots]) => (
                  <div key={day}>
                    <p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: 'var(--dash-muted)' }}>{day}</p>
                      ...
                      {daySlots.map((slot) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            {slot.isBooked && (
                              <span className="text-xs text-amber-500 bg-amber-500/10 rounded-full px-2 py-0.5">Booked</span>
                            )}
                            <button
